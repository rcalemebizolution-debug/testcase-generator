...
export default async function handler(req, res) {
  ...
  try {
    ...
    const imageEvidenceInstruction = imageResult.image
      ? `The attached screenshot is evidence, not decoration. Before drafting cases, privately identify only the clearly visible controls, labels, values, statuses, validation messages, enabled/disabled states, selected options, table columns, and missing or unexpected UI states. Use the written issue details as the source of business intent and the screenshot as the source of UI facts. For every case that relies on the screenshot, anchor the expected result to at least one concrete visible UI fact from that evidence, such as the named control, displayed value, status, error, record, or state change. Quote visible text only when it is legible. Never invent a label, message, API behavior, role, data value, or rule that is not supplied or clearly visible. If the screenshot makes an interpretation uncertain, state that uncertainty as an assumption in the description rather than presenting it as fact.`
      : ''
...
For every case: make the title specific to the risk; state a realistic precondition with required role/data/state; write atomic, executable steps with concrete test data when the issue provides it; and make the expected result observable. Each expected result must be one plain, precise sentence of no more than 45 words, focused only on that scenario's outcome. It must identify the triggering action or condition, the specific UI/data state to verify, and the rule or outcome being asserted. For example, say which field, button, status, row, value, validation feedback, persisted record, or unchanged state proves the behavior. It must directly name the relevant outcome, rule, timing, restriction, or acceptance criterion from the issue description. Never use generic statements such as "the result matches the requirements", "the feature works as expected", "the system handles it correctly", or "the expected behavior occurs." For negative, validation, boundary, and security cases, state exactly what is blocked and what must not be persisted, changed, displayed, or exposed. Across the complete suite, cover every relevant scenario and risk stated in the issue; do not try to combine every scenario into one expected result. Ensure cases do not overlap and each has a unique decision or risk focus. Keep wording concise. Assign sequential IDs starting with TC-001. ${imageEvidenceInstruction} ${outputInstruction}`
    ...
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: createVisionUserContent(`Create a test suite from this ${isDevelopment ? 'feature' : 'issue'} context:\n${JSON.stringify(safeInput, null, 2)}${imageResult.image ? '\n\nUse the attached screenshot as evidence. Make each screenshot-based expected result specific enough that a tester can verify the visible UI or data state without interpreting a generic claim.' : ''}`, imageResult.image) },
        ],
